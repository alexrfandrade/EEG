...
def run_experiment(n_blocks=1):
    ...
    s = serial.Serial('COM3', baudrate=115200, bytesize=serial.EIGHTBITS,
                       ...
                       timeout=1)
    time.sleep(1) # pauses the whole program
    s.write(b'RR') # reset/run
    time.sleep(1)
    s.write(f"{1:02X}".encode()) # send the start trigger (marker for the EEG recording)
    ...
    for block_num in range(1, n_blocks + 1):
        ...
        for trial_num, isOddball in enumerate(trialSequence, 1):
            ...
            # Stimulus trigger
            # s.write(f"{isOddball}".encode)
            if isOddball:
                s.write(str(1).encode()) # oddball=1
            else:
                s.write(str(2).encode()) # standard=2
