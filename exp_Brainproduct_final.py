...
def run_experiment(n_blocks=1):
    ...
    # Start the global clock (before Block 1)
    ...
    for block_num in range(1, n_blocks + 1):
        ...
        for trial_num, isOddball in enumerate(trialSequence, 1):
            ...
            if not isOddball:
                std_index += 1
                win.callOnFlip(send_triggers,b'01') # standard=1 (start)
                # f"{1:02X}".encode() formats 1 as 2-character hex ("01") and encodes to bytes
            else:
                win.callOnFlip(send_triggers,b'02') # oddball=2 (end)
            
            win.flip() # show the stimulus and send the trigger
            tTarget = globalClock.getTime() # record the time of the simulus
            
            # Wait for response
            responded, RT = False, None # no value for the reaction time yet
            while globalClock.getTime() - tTarget < stimDuration: # show the image for 0.5 seconds
